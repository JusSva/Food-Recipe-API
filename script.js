...
function setMessage() {
    result.innerHTML = `<div class="alert alert-warning">We were unable to find any meal recipes based on your query :/</div>`;
}
...
function getSingleMeal(id) {
    ...
    .then(resp => {
        ...
        result.innerHTML = resp.meals.map(value => `
                <div class="col-4">
                    <img src="${value.strMealThumb}">
                    <button class="btn btn-primary mt-4" onclick="backToTheList()">Return</button>
                </div>
                <div class="col-8">
                    <h3>${value.strMeal}</h3>
